...
/**
 * * 作为MyBatis工作的主要顶层API，表示和数据库交互的会话，完成必要数据库增删改查功能 mybatis中的设计模式
 * 工厂模式SqlSessionFactory/ObjectFactory/MapperProxyFactory
 * 建造模式SqlSessionFactoryBuilder/XMLConfigBuilder/XMLMapperBuilder/XMLStatementBuilder/CacheBuilder,Environment,把建造的步骤分装到一个类里,且运用了fluent
 * API模式。 单例模式LogFactory,这个简单，不许new多个实例
 * 合成模式MixedSqlNode，里面有许多孩子，TextSqlNode,ForEachSqlNode,IfSqlNode....
 * 装饰模式Cache，一个个缓存类通过一个链条串起来 代理模式MapperProxy/ConnectionLogger,用的jdk的动态代理
 ...
 * 适配器模式，logging包统一了各大日志框架的接口 The primary Java interface for working with
 * MyBatis. Through this interface you can execute commands, get mappers and
 * manage transactions.
 * 
 * @author liuzhongda
 *
